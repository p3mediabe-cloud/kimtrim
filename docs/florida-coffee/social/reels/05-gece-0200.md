# Reel 05 — "Şehir Uyurken"

| | |
|---|---|
| **Pijler** | Manzara / mekan |
| **Persona** | Selin & Can — secundair Emre (late dienst) |
| **Platform** | Instagram Reels |
| **Vestiging** | Beykoz |
| **Duur** | 16 s |
| **Doel** | Bezit nemen van een bezoekmoment dat niemand claimt: de late avond. |

## Concept

Elke koffieketen claimt de ochtend. Bijna niemand de nacht. Beykoz is 's avonds laat open
en kijkt uit over het water — dat is een onderscheidend bezoekmoment en een zoekintentie
die nu onbeheerd is (§3.3, fırsat pencereleri). Warme gloed tegen natte straat.

```
SLAG 1 — HAAK    0–2 s    Amberkleurig raam in een donkere straat, natte stoep die weerkaatst.
SLAG 2 — MOMENT  2–12 s   Lateral dolly langs de gevel, iemand loopt voorbij met een beker.
SLAG 3 — LANDING 12–16 s  Binnen: filterkoffie druppelt, stad donker achter het glas.
```

## Shotlist

| # | Beeld | Camera | Licht | Duur | First frame |
|---|---|---|---|---|---|
| 1 | Verlicht raam, natte straat weerkaatst amber | Statisch, micro-drift | Nacht, amber uit het raam | 8 s | `img/branded/night-9x16.jpg` |
| 2 | Trage zijwaartse dolly langs de gevel, figuur passeert | Lateral dolly links | Nacht, koplampen-bokeh | 8 s | `img/branded/night-9x16.jpg` |
| 3 | Filterkoffie druppelt, donkere Bosporus achter glas | Trage push-in | Amber lamp, koud blauw buiten | 8 s | `img/branded/gecefiltre-9x16.jpg` |

## Google Flow — prompts

**Instellingen:** 9:16 · 720p · 8s · Veo 3.1 · modus **Frames to Video**

### Clip 1 — `01-vitrin` · startbeeld `img/branded/night-9x16.jpg`
```
Static shot with the faintest drift: a cafe window glows warm amber at night on a quiet street
near the water. Rain-wet pavement in the foreground mirrors the light in long vertical streaks.
Deep teal and near-black surroundings, a few warm highlights. Calm, cinematic, nobody in frame yet.
Brand: Florida Coffee, Istanbul. Palette petrol teal #004854, amber #F09C1C, rust #D44808, cream.
One continuous shot, no cuts, no scene changes, loopable. Photoreal, cinematic, natural light,
shallow depth of field, no added text, no captions, no talking, no people looking at camera.
The Florida Coffee logo in the upper-left corner and the toucan mark on the cup are part of the
scene: keep them exactly as they are — same position, size, shape and colour — sharp, flat and
unchanged for the entire shot. Never redraw, animate, warp, relight or remove them, and add no
second logo or invented lettering anywhere in frame.
Ambient only: light rain, distant traffic, faint water — no voices, no music.
```

### Clip 2 — `02-gecen` · startbeeld `img/branded/night-9x16.jpg`
```
Slow lateral dolly to the left past the glowing facade of a night cafe. A person in a long coat
walks through frame carrying a cream cup, seen from behind and in silhouette, never facing camera.
Car headlights pass in the background as soft bokeh. Rain-wet pavement reflects amber light.
Brand: Florida Coffee, Istanbul. Palette petrol teal #004854, amber #F09C1C, rust #D44808, cream.
One continuous shot, no cuts, no scene changes, loopable. Photoreal, cinematic, natural light,
shallow depth of field, no added text, no captions, no talking, no people looking at camera.
The Florida Coffee logo in the upper-left corner and the toucan mark on the cup are part of the
scene: keep them exactly as they are — same position, size, shape and colour — sharp, flat and
unchanged for the entire shot. Never redraw, animate, warp, relight or remove them, and add no
second logo or invented lettering anywhere in frame.
Ambient only: footsteps on wet pavement, passing car, light rain — no voices, no music.
```

### Clip 3 — `03-filtre` · startbeeld `img/branded/gecefiltre-9x16.jpg`
```
Slow push-in on a glass pour-over carafe on a wooden counter at night: dark coffee drips steadily
from the filter cone, steam rising. Behind it, a large window shows the dark water and scattered
distant city lights out of focus. Warm amber lamp above, cool blue night outside. Quiet, intimate.
Brand: Florida Coffee, Istanbul. Palette petrol teal #004854, amber #F09C1C, rust #D44808, cream.
One continuous shot, no cuts, no scene changes, loopable. Photoreal, cinematic, natural light,
shallow depth of field, no added text, no captions, no talking, no people looking at camera.
The Florida Coffee logo in the upper-left corner and the toucan mark on the cup are part of the
scene: keep them exactly as they are — same position, size, shape and colour — sharp, flat and
unchanged for the entire shot. Never redraw, animate, warp, relight or remove them, and add no
second logo or invented lettering anywhere in frame.
Ambient only: slow dripping, faint room tone — no voices, no music.
```

**Negatief (alle clips):**
```
subtitles, captions, invented lettering, second logo, duplicated watermark, missing logo,
distorted hands, extra fingers, cartoon, oversaturated, plastic skin, stock-photo smile,
lens flare, fast camera shake
```

## Turkse copy

**On-screen (3 kaarten):**
| Tijd | Tekst |
|---|---|
| 0,6 s | `Şehir uyudu.` |
| 6,0 s | `Burası uyanık.` |
| 13,0 s | `Gece filtresi.` |

**Caption — Reels:**
```
Şehir uyudu. Burası uyanık.

Islak kaldırım ışığı tutuyor.
Cam kenarında su karanlık.
Bir filtre daha demleniyor.

Geç saatte bir köşe arıyorsan, biliyorsun ☕

📍 Florida Coffee Beykoz
#floridacoffee #mutluluğuntadı #kahvekeyfi #üçüncüdalga #beykoz #boğaz #gecekahvesi #filtrekahve
```

## Montage

```
Clips      : 01-vitrin.mp4 (0–2 s) · 02-gecen.mp4 (2–12 s) · 03-filtre.mp4 (12–16 s)
Tekstkaart : 0,6 s  "Şehir uyudu."   — cream, onderin veilige zone
Tekstkaart : 6,0 s  "Burası uyanık." — amber
Tekstkaart : 13,0 s "Gece filtresi."
Logo       : logo-reverse.png linksboven, vanaf 0,0 s — nachtscene
Eindkaart  : 14,5–16 s  logo-reverse.png op #004854
Geluid     : regen + straat uit de clips dragend, laag ambient bed -20 dB
Export     : 1080×1920, H.264, ±8 Mbps
```

## Aannames & te verifiëren

- **Het sluitingsuur is bewust níet in beeld gezet.** Analyse §2.2 noemt "02:00'a kadar
  açık" voor Beykoz, maar die lijst is expliciet nog niet door de klant geverifieerd
  (§11.2). Zodra HQ bevestigt, is `Gece 02:00'a kadar` een sterke extra tekstkaart —
  dat concrete uur is precies wat deze reel deelbaar maakt.
