...
| `site/build.py --public <kök URL>` | Paylaşım kartları: her sayfa için 1200×630 markalı `img/og/<ad>.jpg` (fotoğraf + gradyan + logo) ve mutlak `og:image` / `twitter:card` etiketleri; Pages akışı `--public https://p3mediabe-cloud.github.io/kimtrim` geçer. Pillow gerekir |
...
| `demo-site/menu.json` | Menünün tek kaynağı: 53 ürün, 4 kategori; her üründe kaynak etiketi (`elkitabi` = barista el kitabı, `listing` = Yemeksepeti özeti, `varsayim` = müşteriyle doğrulanacak) ve fotoğraf çekim tarifi |
| `demo-site/menu_tools.py` | `sync`: menu.json'u demonun `MENU` bloğuna yazar · `images`: her ürün için `img/menu/<slug>.jpg` üretir (Nano Banana Pro, tek stil rehberi, 1:1) |
| `demo-site/gen_video.py` | Veo 3.1 ile hero videosu; ilk kare kendi marka görselimizden, hedef orana kırpılır. `GEMINI_API_KEY` ortam değişkeninden |
| `demo-site/brand/` | Franchise sunumundan çıkarılan gerçek logo: orijinal, koyu zemin için krem sürüm ve "o" harfi delinmiş kelime işaretleri (animasyonlu SVG işaret üstüne oturur) |
...
Ana sayfa hikâyesi, Flo motoru ve şube verisi `demo-site/index.html`'den, menü `demo-site/menu.json`'dan (önce `python3 menu_tools.py sync`), alt sayfa içerikleri `build.py` içinden gelir.
